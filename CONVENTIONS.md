...
- Idiomatic C++20. Use the language; do not reinvent what the standard library, asio, or
  other dependencies already do well.
...
Avoid:
- **Salami-slicing** — carving a cohesive unit into fragments just to fit the limit.
- **One-function files** — files that exists only to hold a single function but are still composed together by the consumer.
- **Artificial purity** — a wrapper or layer that adds indirection without separating a real responsibility.
