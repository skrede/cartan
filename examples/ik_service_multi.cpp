...
///
/// Unlike TRAC-IK which spawns a thread per solver strategy, liepp's
...
liepp::kinematic_chain<double, 7> make_lbr_iiwa()
{
    using vec3 = liepp::vector3<double>;

    auto s1 = liepp::screw_axis<double>::revolute(vec3(0, 0, 1), vec3(0, 0, 0));
    auto s2 = liepp::screw_axis<double>::revolute(vec3(0, 1, 0), vec3(0, 0, 0.360));
    auto s3 = liepp::screw_axis<double>::revolute(vec3(0, 0, 1), vec3(0, 0, 0.360));
    auto s4 = liepp::screw_axis<double>::revolute(vec3(0, -1, 0), vec3(0, 0, 0.780));
    auto s5 = liepp::screw_axis<double>::revolute(vec3(0, 0, 1), vec3(0, 0, 0.780));
    auto s6 = liepp::screw_axis<double>::revolute(vec3(0, 1, 0), vec3(0, 0, 1.180));
    auto s7 = liepp::screw_axis<double>::revolute(vec3(0, 0, 1), vec3(0, 0, 1.180));
    ...
    return liepp::kinematic_chain<double, 7>(
...
struct ik_response
{
    std::expected<liepp::ik_result<double, 7>, liepp::ik_error<double, 7>> result;
...
//   liepp:   M fixed worker threads -> O(M) threads total, regardless of policy count

class ik_service_pool
{
public:
    ik_service_pool(liepp::kinematic_chain<double, 7> chain, int num_workers)
...
private:
    ...
    liepp::kinematic_chain<double, 7> m_chain;
    std::mutex m_mutex;
...
int main()
{
    ...
    {
        auto target = liepp::forward_kinematics(chain, configs[static_cast<std::size_t>(i)]).end_effector;
