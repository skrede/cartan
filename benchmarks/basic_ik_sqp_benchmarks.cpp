...
static void bm_ik_nlopt_bobyqa_3r_planar(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        ...
        cartan::basic_ik_runner<cartan::ik::nlopt_bobyqa<cartan::kinematic_chain<double, 3>>> solver;
...
BENCHMARK(bm_ik_nlopt_bobyqa_3r_planar)->Iterations(10000)->Unit(benchmark::kMicrosecond);

static void bm_ik_nlopt_bobyqa_ur3e(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        ...
        cartan::basic_ik_runner<cartan::ik::nlopt_bobyqa<cartan::kinematic_chain<double, 6>>> solver;
        solver.setup(chain, target, q_seed, criteria);
        auto result = solver.solve();
...
BENCHMARK(bm_ik_nlopt_bobyqa_ur3e)->Iterations(10000)->Unit(benchmark::kMicrosecond);

static void bm_ik_nlopt_bobyqa_lbr_med14(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        ...
        cartan::basic_ik_runner<cartan::ik::nlopt_bobyqa<cartan::kinematic_chain<double, 7>>> solver;
        solver.setup(chain, target, q_seed, criteria);
        auto result = solver.solve();
...
BENCHMARK(bm_ik_nlopt_bobyqa_lbr_med14)->Iterations(10000)->Unit(benchmark::kMicrosecond);
