...
public:
    ...
    step_result<scalar_type> step(const Chain& chain, int N)
    {
        ...
        {
            // One algorithmic-work unit = one argmin-internal LM iteration.
            // Drive the inner solver one iteration at a time so the cartan-side
            // per-attempt counter and the runner-side total-budget counter
            // both track inner algorithmic work units 1:1.
            ...
            if (inner_units <= 0)
            {
                // Inner solver did no work (already terminal). Treat as
                // stalled to avoid an infinite loop.
            ...
            if (cartan::detail::is_converged_unweighted(V_b, m_criteria))
            {
                ...
                // pose only reachable outside the box is a joint-limit failure.
                // Canonicalizes the FINAL synced iterate once; never a clamp of
                // the argmin working state.
