...
public:
    ...
    nlopt_bobyqa() = default;

    explicit nlopt_bobyqa(const options& opts)
        : m_options(opts)
    {
    }
    ...
    /// Execute up to N BOBYQA optimization rounds.
    ///
    /// One algorithmic-work unit = one NLopt BOBYQA optimize() call (each
