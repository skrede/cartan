...
public:
    ...
    step_result<scalar_type> step(const Chain& chain, int N)
    {
        ...
        {
            // One algorithmic-work unit = one argmin-internal CMA-ES generation.
            auto prev_iter = m_solver->state().generation;
            auto result = m_solver->step_n(1);
            auto inner_units = static_cast<int>(m_solver->state().generation - prev_iter);
            if (inner_units <= 0)
            {
