...
| benchmarks/basic_ik_full_benchmarks.cpp | 1223 | one benchmark family over a fixture built once at the top of the file and read by every case beneath it; a split would either duplicate the fixture or export it, and a duplicated fixture makes two runs incomparable |
...
| python/src/bindings/ik_bindings.cpp | 416 | one extension module's binding surface, where a type must be registered before any signature that mentions it; a split would spread that ordering constraint across files with nothing left to enforce it |
