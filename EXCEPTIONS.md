...
| lib/cartan-serial-chain/include/cartan/serial/ik/basic_ik_runner.h | 730 | the runner and its variadic multi-policy overloads are one class template; the policy pack is threaded through every member, so a split would repeat the pack expansion in each half |
...
| lib/cartan-serial-chain/include/cartan/serial/fk/detail/axis_specializations.h | 447 | the per-axis specializations are one dispatch table; a split would scatter the joint tags across headers and make an unspecialized tag a link error rather than a compile error |
| lib/cartan-urdf/include/cartan/urdf/build.h | 440 | one ordered walk from a parsed model to a chain, with the two entry points that read what it accumulated: the leaf, branch and fold cases are all decided against the same accumulator, and separating them would leave each fragment a half-walked model to finish |
...
| lib/cartan-serial-chain/include/cartan/serial/ik/solver/lm.h | 331 | one solver class whose state, step and termination bookkeeping are read together; splitting it puts the invariant tying a step to its convergence test in one header and the state it reads in another |
...
| lib/cartan-serial-chain/include/cartan/serial/ik/detail/limit_enforcement.h | 282 | the joint-limit predicates and the canonicalization that repairs a solution share one definition of feasibility; splitting the test from the repair lets the two disagree about the same joint |
| lib/cartan-serial-chain/include/cartan/serial/ik/ik_status.h | 299 | the status, objective, failure and termination enumerations with the message functions that render them; a reader adding a value has to touch the enumeration and its renderer together, and separating them is how the two drift apart |
...
| tests/unit/honest_diagnostics_test.cpp | 402 | three chain fixtures -- a six-joint arm, a duplicated-axis chain that is exactly singular, and one carrying a non-finite bound -- are declared once above the cases and read across both halves; the measures and the diagnostics they replaced are the same subject seen from two sides, and splitting them duplicates the fixtures into copies that drift |
| tests/unit/selection_objective_test.cpp | 497 | four chain fixtures and a race helper are declared once above the cases and read by nearly all of them; splitting the cases duplicates those fixtures, and an adversarial pair only discriminates against the exact chain it was measured on, so drifted copies would assert the same sentence about different geometry |
...
| tests/unit/jacobian_sweep_test.cpp | 377 | a case list over one subject whose fixtures, helpers and tolerance constants are declared once above the cases and used by all of them; splitting it duplicates that setup into each fragment, where the copies drift and two files then test against different fixtures |
...
| tests/unit/jacobian_test.cpp | 348 | a case list over one subject whose fixtures, helpers and tolerance constants are declared once above the cases and used by all of them; splitting it duplicates that setup into each fragment, where the copies drift and two files then test against different fixtures |
...
| tests/unit/forward_kinematics_test.cpp | 325 | a case list over one subject with no shared setup to scatter; the harm in splitting is to the reader, who would then have to know which fragment holds a behavior before being able to tell whether it is covered at all |
...
| tests/unit/lm_bit_identity_test.cpp | 276 | the panel definition, the flattened blob type and the bit emitters are shared by the comparison case and by the hidden generator that writes the record the comparison reads; splitting them separates a generator from the consumer it has to agree with bit for bit, and drifted copies would compare against a record they did not produce |
...
| Path | Lines at registration | Why splitting would harm |
| --- | --- | --- |
| benchmarks/basic_ik_full_benchmarks.cpp | 1220 | one benchmark family over a fixture built once at the top of the file and read by every case beneath it; a split would either duplicate the fixture or export it, and a duplicated fixture makes two runs incomparable |
| benchmarks/ik_comparison_pinocchio_benchmarks.cpp | 933 | one benchmark family over a fixture built once at the top of the file and read by every case beneath it; a split would either duplicate the fixture or export it, and a duplicated fixture makes two runs incomparable |
...
| benchmarks/benchmark_utils.h | 284 | the chain definitions for every compared library in one place, so a robot's cartan and external descriptions sit next to each other; splitting by library is how the two descriptions of one robot silently stop matching |
