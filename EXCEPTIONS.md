...
| lib/cartan-serial-chain/include/cartan/serial/ik/basic_ik_runner.h | 712 | the runner and its variadic multi-policy overloads are one class template; the policy pack is threaded through every member, so a split would repeat the pack expansion in each half |
...
| lib/cartan-serial-chain/include/cartan/serial/ik/wrapper/restart_wrapper.h | 423 | one wrapper whose restart policy, seed generation and best-result bookkeeping are a single loop; a split would export the loop's working state so a second file could read it |
...
| tests/unit/variadic_solver_test.cpp | 481 | a case list over one subject whose fixtures, helpers and tolerance constants are declared once above the cases and used by all of them; splitting it duplicates that setup into each fragment, where the copies drift and two files then test against different fixtures |
| tests/unit/honest_diagnostics_test.cpp | 276 | three chain fixtures -- a six-joint arm, a duplicated-axis chain that is exactly singular, and one carrying a non-finite bound -- are declared once above the cases and read across both halves; the measures and the diagnostics they replaced are the same subject seen from two sides, and splitting them duplicates the fixtures into copies that drift |
| tests/unit/selection_objective_test.cpp | 462 | four chain fixtures and a race helper are declared once above the cases and read by nearly all of them; splitting the cases duplicates those fixtures, and an adversarial pair only discriminates against the exact chain it was measured on, so drifted copies would assert the same sentence about different geometry |
| tests/boundary/solver_setup_boundary_test.cpp | 482 | a case list over one subject whose fixtures, helpers and tolerance constants are declared once above the cases and used by all of them; splitting it duplicates that setup into each fragment, where the copies drift and two files then test against different fixtures |
