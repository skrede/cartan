...
| lib/cartan-serial-chain/include/cartan/serial/ik/wrapper/restart_wrapper.h | 414 | one wrapper whose restart policy, seed generation and best-result bookkeeping are a single loop; a split would export the loop's working state so a second file could read it |
| lib/cartan-serial-chain/include/cartan/serial/ik/solver/argmin_projected_gradient_gn.h | 412 | one solver class whose state, step and termination bookkeeping are read together; splitting it puts the invariant tying a step to its convergence test in one header and the state it reads in another |
| lib/cartan-serial-chain/include/cartan/serial/ik/solver/filter_nw_sqp.h | 401 | one solver class whose state, step and termination bookkeeping are read together; splitting it puts the invariant tying a step to its convergence test in one header and the state it reads in another |
...
| lib/cartan-serial-chain/include/cartan/serial/ik/solver/gcmma.h | 250 | one solver class whose state, step and termination bookkeeping are read together; splitting it puts the invariant tying a step to its convergence test in one header and the state it reads in another |
...
| lib/cartan-serial-chain/include/cartan/serial/ik/solver/newton_raphson.h | 247 | one solver class whose state, step and termination bookkeeping are read together; splitting it puts the invariant tying a step to its convergence test in one header and the state it reads in another |
...
| lib/cartan-serial-chain/include/cartan/serial/ik/solver/cmaes.h | 242 | one solver class whose state, step and termination bookkeeping are read together; splitting it puts the invariant tying a step to its convergence test in one header and the state it reads in another |
| lib/cartan-serial-chain/include/cartan/serial/ik/solver/argmin_bobyqa.h | 235 | one solver class whose state, step and termination bookkeeping are read together; splitting it puts the invariant tying a step to its convergence test in one header and the state it reads in another |
| lib/cartan-serial-chain/include/cartan/serial/ik/solver/augmented_lagrangian.h | 235 | one solver class whose state, step and termination bookkeeping are read together; splitting it puts the invariant tying a step to its convergence test in one header and the state it reads in another |
