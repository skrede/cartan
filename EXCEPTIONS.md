...
| lib/cartan-serial-chain/include/cartan/serial/ik/basic_ik_runner.h | 697 | the runner and its variadic multi-policy overloads are one class template; the policy pack is threaded through every member, so a split would repeat the pack expansion in each half |
...
| python/src/bindings/ik_bindings.cpp | 410 | one extension module's binding surface, where a type must be registered before any signature that mentions it; a split would spread that ordering constraint across files with nothing left to enforce it |
