...
TEST_CASE("framed_wrench: coadjoint_map frame propagation", "[framed_wrench]")
{
    ...
    cartan::vector3<double> moment;
    moment << 0.0, 0.0, 1.0;
    cartan::vector3<double> force;
    force << 1.0, 0.0, 0.0;
    auto fw = cartan::framed_wrench<base>::from_moment_force(moment, force);

    // Apply coadjoint map
    auto result = cartan::coadjoint_map(T, fw);

    // Result type should be framed_wrench<world>
    static_assert(std::is_same_v<
        decltype(result),
        cartan::framed_wrench<world, double>>);

    // Numerical check: matches raw CoAd_T * W
    cartan::matrix6<double> CoAd = T.m_value.coadjoint();
    cartan::vector6<double> expected = CoAd * fw.m_value;
...
{
    cartan::vector3<double> m;
    m << 1.0, 2.0, 3.0;
    cartan::vector3<double> f;
    f << 4.0, 5.0, 6.0;
    auto fw = cartan::framed_wrench<world>::from_moment_force(m, f);
...
TEST_CASE("framed_wrench: coadjoint_map numerical match", "[framed_wrench]")
{
    ...
    // Non-trivial wrench
    cartan::vector6<double> wrench_vec;
    wrench_vec << 0.2, -0.4, 0.6, 1.0, -0.5, 0.3;
    auto fw = cartan::framed_wrench<base>{wrench_vec};

    auto result = cartan::coadjoint_map(T, fw);

    // Compare with raw computation
    cartan::vector6<double> expected = T.m_value.coadjoint() * wrench_vec;
    REQUIRE((result.m_value - expected).norm() < 1e-12);
}
...
TEST_CASE("framed_wrench: m_value is accessible as vector6", "[framed_wrench]")
{
    ...
    auto fw = cartan::framed_wrench<world>{v};
