...
{
    static_assert(spp::solve_policy<cartan_examples::nlopt_slsqp<spp::kinematic_chain<double, 6>>>);
    SUCCEED();
}
...
TEST_CASE("nlopt_slsqp_solve_policy converges on UR5-like chain", "[ik][slsqp]")
{
    ...
    cartan_examples::nlopt_slsqp<spp::kinematic_chain<double, 6>> stepper;
...
TEST_CASE("nlopt_slsqp_solve_policy composes with restart_solve_policy", "[ik][slsqp]")
{
    ...
    using inner_t = cartan_examples::nlopt_slsqp<spp::kinematic_chain<double, 6>>;
