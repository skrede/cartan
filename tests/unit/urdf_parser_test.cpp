...
{
    auto result = cartan::urdf::parse_urdf_file<double>(fixture_path("parser_minimal.urdf"));
    ...
    CHECK(j1.kind == cartan::urdf::parsed_joint_kind::revolute);
    ...
    CHECK(j2.kind == cartan::urdf::parsed_joint_kind::continuous);
...
{
    auto result = cartan::urdf::parse_urdf_file<double>(fixture_path("parser_unclosed.urdf"));

    REQUIRE_FALSE(result.has_value());
    CHECK(result.error().kind == cartan::urdf::urdf_failure::malformed_xml);
...
{
    auto result = cartan::urdf::parse_urdf_file<double>(fixture_path("parser_unknown_joint.urdf"));

    REQUIRE_FALSE(result.has_value());
    CHECK(result.error().kind == cartan::urdf::urdf_failure::unsupported_joint_type);
...
{
    auto result = cartan::urdf::parse_urdf_file<double>(fixture_path("parser_unknown_parent.urdf"));

    REQUIRE_FALSE(result.has_value());
    CHECK(result.error().kind == cartan::urdf::urdf_failure::unknown_parent_link);
...
{
    auto result = cartan::urdf::parse_urdf_file<double>(fixture_path("parser_mimic.urdf"));
