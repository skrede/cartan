...
TEMPLATE_TEST_CASE("se3: exp/log roundtrip", "[se3]", double, float)
{
    ...
    SECTION("large rotation (theta near pi)")
    {
        ...
        auto T_back = liepp::se3<S>::exp(v_back);
...
{
    auto T = liepp::se3<double>::exp(liepp::vector6<double>::Zero());
...
TEST_CASE("se3: compose matches matrix product", "[se3]")
{
    ...
    auto a = liepp::se3<double>::exp(v_a);
    auto b = liepp::se3<double>::exp(v_b);
    auto composed = a * b;
...
TEST_CASE("se3: adjoint identity numerical check", "[se3]")
{
    ...
    auto T = liepp::se3<double>::exp(v_T);
    ...
    liepp::matrix4<double> result = T.matrix() * V_hat * T.inverse().matrix();
...
TEST_CASE("se3: from_matrix with valid SE(3)", "[se3]")
{
    ...
    auto T = liepp::se3<double>::exp(v);
    auto result = liepp::se3<double>::from_matrix(T.matrix());
...
{
    liepp::matrix4<double> bad = liepp::matrix4<double>::Identity();
    bad(3, 0) = 0.5;
    auto result = liepp::se3<double>::from_matrix(bad);
...
TEST_CASE("se3: rotation and translation accessors", "[se3]")
{
    ...
    REQUIRE((RtR - liepp::matrix3<double>::Identity()).norm() < 1e-12);
...
TEST_CASE("se3: strict vs fast policy", "[se3][policy]")
{
    ...
    auto T_strict = liepp::se3<double, liepp::strict_policy>::exp(v);
    auto T_fast = liepp::se3<double, liepp::fast_policy>::exp(v);
...
TEST_CASE("se3: mixed-policy compose returns strict result", "[se3][policy]")
{
    ...
    auto strict_T = liepp::se3<double, liepp::strict_policy>::exp(v_a);
    auto fast_T = liepp::se3<double, liepp::fast_policy>::exp(v_b);

    auto result = strict_T * fast_T;

    static_assert(std::is_same_v<
        decltype(result),
        liepp::se3<double, liepp::strict_policy>>);
...
TEST_CASE("se3: float scalar operations", "[se3][float]")
{
    ...
    auto T = liepp::se3<float>::exp(v);
    auto v_back = T.log();
    REQUIRE((v_back - v).norm() < 1e-3f);

    auto inv = T.inverse();
    auto product = T * inv;
    auto I = liepp::matrix4<float>::Identity();
...
TEST_CASE("se3: log with pure rotation (zero translation)", "[se3]")
{
    ...
    auto rot = liepp::so3<double>::exp(liepp::vector3<double>(0.5, -0.3, 0.8));
    auto T = liepp::se3<double>(rot, liepp::vector3<double>::Zero());

    auto v = T.log();

    // Omega part should match the so3 log
    liepp::vector3<double> omega = v.head<3>();
    liepp::vector3<double> expected_omega = rot.log();
