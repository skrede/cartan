...
// "[.capture]" case writes lm_golden_data.h next to this source), and the
// always-run comparison case then re-derives the same outputs and asserts
...
template <typename LimitsPolicy = cartan::no_limits, typename Scalar, int N>
void run_chain(
    ...
    std::vector<int>& ints_out)
{
    ...
    for (int t = 0; t < panel_targets; ++t)
    {
        ...
        cartan::lm<cartan::kinematic_chain<Scalar, N>, LimitsPolicy> stepper;
...
golden_blob build_blob()
{
    ...
    // The trailing two entries per scalar type use a limits policy that moves
    // the iterate after the step, which is a different path through the solver
    // than the default no-op policy. Seven joints are what reach the null-space
    // projection: on a non-singular six-joint arm only the trailing clamp runs.
    run_chain<cartan::clamp_limits>(
        cartan::fixtures::make_ur3e_chain<double>(), blob.double_scalars, blob.double_ints);
    run_chain<cartan::null_space_limits>(
        cartan::fixtures::make_lbr_med14_chain<double>(), blob.double_scalars, blob.double_ints);
    ...
    run_chain<cartan::clamp_limits>(
        cartan::fixtures::make_ur3e_chain<float>(), blob.float_scalars, blob.float_ints);
    run_chain<cartan::null_space_limits>(
        cartan::fixtures::make_lbr_med14_chain<float>(), blob.float_scalars, blob.float_ints);
...
#if __has_include("lm_golden_data.h")
#include "lm_golden_data.h"
#define CARTAN_LM_HAVE_GOLDEN 1
#else
#define CARTAN_LM_HAVE_GOLDEN 0
#endif

#if CARTAN_LM_HAVE_GOLDEN

TEST_CASE("lm outputs are bitwise identical to the golden record", "[ik][lm][golden]")
{
    ...
    REQUIRE(blob.double_scalars.size() == cartan_lm_golden::double_scalar_count);
    REQUIRE(blob.double_ints.size() == cartan_lm_golden::double_int_count);
    REQUIRE(blob.float_scalars.size() == cartan_lm_golden::float_scalar_count);
    REQUIRE(blob.float_ints.size() == cartan_lm_golden::float_int_count);
    ...
    for (std::size_t i = 0; i < blob.double_scalars.size(); ++i)
    {
        ...
        REQUIRE(bits == cartan_lm_golden::double_scalar_bits[i]);
    }
    for (std::size_t i = 0; i < blob.double_ints.size(); ++i)
    {
        REQUIRE(blob.double_ints[i] == cartan_lm_golden::double_ints[i]);
    }
    for (std::size_t i = 0; i < blob.float_scalars.size(); ++i)
    {
        const auto bits = std::bit_cast<std::uint32_t>(blob.float_scalars[i]);
        REQUIRE(bits == cartan_lm_golden::float_scalar_bits[i]);
    }
    for (std::size_t i = 0; i < blob.float_ints.size(); ++i)
    {
        REQUIRE(blob.float_ints[i] == cartan_lm_golden::float_ints[i]);
    }
}
...
TEST_CASE("capture lm bitwise golden record", "[.capture]")
{
#ifndef CARTAN_LM_GOLDEN_DIR
    FAIL("CARTAN_LM_GOLDEN_DIR not defined; cannot write golden record");
#else
    const golden_blob blob = build_blob();
    ...
    // The record wraps wider than the projected-solver record beside it because
    // this panel is larger, and at four and six values per line the generated
    // file would carry more lines than the file-size ceiling admits.
    ...
    body << "#ifndef HPP_GUARD_CARTAN_TESTS_UNIT_LM_GOLDEN_DATA_H\n";
    body << "#define HPP_GUARD_CARTAN_TESTS_UNIT_LM_GOLDEN_DATA_H\n\n";
    body << "// Auto-generated bitwise golden record for the lm solver.\n";
    body << "// Regenerate with the \"[.capture]\" test case. Do not edit by hand.\n\n";
    body << "#include <cstddef>\n#include <cstdint>\n\n";
    body << "namespace cartan_lm_golden\n{\n\n";
    ...
    const std::string path = std::string(CARTAN_LM_GOLDEN_DIR) + "/lm_golden_data.h";
