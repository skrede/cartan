    ...
    "[ik][boundary]", double, float)
{
    ...
    auto result = runner.solve();
    REQUIRE_FALSE(result.has_value());
    // A refused setup measured neither the residual nor an iterate, so both
    // fields keep their poison. The largest representable value and the zero
    // vector that stood here before read as a measured distance and as the home
    // configuration, and were asserted as the contract by these very lines.
    REQUIRE(std::isnan(result.error().last_error_norm));
    REQUIRE(result.error().last_q.size() == n);
    REQUIRE(result.error().last_q.array().isNaN().all());
    REQUIRE(runner.current_q().array().isNaN().all());
